...
- Command: `python external/LHM_3dnav/render_gs_from_img_smplx.py --images <img_dir> --motion <motion_dir> --export-mesh [--no-export-gs] [--zero-hands]`
...
- Default run (no `--export-mesh`) renders the GS sequence: `python external/LHM_3dnav/render_gs_from_img_smplx.py --images <img_dir> --motion <motion_dir> [--export-video] [--zero-hands]`
...
- `--zero-hands`: force closed/flat hands on every motion frame (overrides existing hand poses).
